...
def test_raw_insert_simple():
    """Tests the raw_insert() function that is used to insert values into the database."""
    table_name = "test"
    sql = build_insert_query(table_name, ["filename", "id"], [("final_final_test", 5000)])
...
def test_raw_insert_bulk():
    """Tests the raw_insert() function that is used to insert values into the database."""
    table_name = "test"
    sql = build_insert_query(table_name, ["filename", "id"], [("test_1", 5000), ("test_2", 10000), ("test_3", 1000)])
...
def test_pandas_insert():
    ...
    # table_name = "test"
    data_frame = pd.DataFrame([["test_5", 5], ["test_6", 6],["test_7", 7],["test_8", 8] ], columns=["filename", "id"])
